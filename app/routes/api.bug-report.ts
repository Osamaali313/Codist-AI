...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.boltVersion) {
      body += `- Codist AI: ${data.environmentInfo.boltVersion}\n`;
    }
  ...
  body += '---\n*Submitted via Codist AI bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'Osamaali313/Codist-AI';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'codist-ai-bug-reporter',
    });
